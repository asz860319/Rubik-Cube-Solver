...
public class RubikCubeVisualizerRefresher2D implements RubikCubeVisualizerRefresher
{
	...
	public void refresh() 
	{
		...
		RubikCubeVisualizerSide currentRight = visor.getSideByCode(
											   currentLeft.getOppositeCubeSideTypeCode());
		RubikCubeVisualizerSide currentBack  = visor.getSideByCode(
											   visor.getCurrentFront().getOppositeCubeSideTypeCode());
		RubikCubeVisualizerSide currentDown  = visor.getSideByCode(
											   visor.getCurrentUp().getOppositeCubeSideTypeCode());
		...
		visor.refresh.showSide(currentRight, startX + (sideSize + space) * 2, startY, 
				currentBack,  5, sideSize, sideSize, 0, 0);
		
		visor.refresh.showSide(currentBack,  startX + (sideSize + space) * 3, startY, 
				currentLeft,  5, sideSize, sideSize, 0, 0);
		
		visor.refresh.showSide(visor.getCurrentUp(), startX + (sideSize + space) * 1, 
				startY - sideSize - space * 1, 
				currentBack,  1, sideSize, sideSize, 0, 0);
		
		visor.refresh.showSide(currentDown,  startX + (sideSize + space) * 1, startY + sideSize + space * 1, 
